...
import { applyBuyback } from './actions';
...
export function BuybackClient({
  ...
  async function handleApply(listing: EligibleListing) {
    setApplying(true);
    setError(null);
    const res = await applyBuyback(listing.id);
    setApplying(false);
    if (res?.error) {
      setError(res.error);
      return;
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8">
              ...
              <div key={l.id} className="card p-5">
                ...
                <div className="grid grid-cols-3 gap-2 mb-4 text-sm">
                  ...
                  <div className="rounded-lg bg-emerald-50 p-2 text-center">
                    ...
                    <p className="font-bold text-emerald-700">{formatYen(l.buyback_price)}</p>
                  </div>
                </div>
                <button
                  onClick={() => { setError(null); setConfirmId(l.id); }}
              ...
              return (
                <div key={r.id} className={`card border p-4 ${r.status === 'approved' ? 'border-emerald-200' : ''}`}>
                  ...
                  {r.status === 'rejected' && r.rejection_reason && (
                    <p className="mt-2 rounded-lg bg-red-50 p-2 text-xs text-red-600">理由: {r.rejection_reason}</p>
                  )}
                </div>
              );
            })}
      ...
      {/* 確認モーダル */}
      {confirmId && (() => {
        const l = eligible.find((x) => x.id === confirmId);
        if (!l) return null;
        return (
            ...
            <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl"
              ...
              <div className="rounded-xl bg-slate-50 p-4 mb-5 space-y-2 text-sm">
                ...
                <div className="flex justify-between border-t pt-2 mt-1">
                  ...
                  <span className="text-xl font-black text-emerald-600">{formatYen(l.buyback_price)}</span>
                </div>
              </div>
              <p className="text-xs text-slate-400 mb-3">申請後、担当者が実車確認を経て最終金額を確定します。</p>
